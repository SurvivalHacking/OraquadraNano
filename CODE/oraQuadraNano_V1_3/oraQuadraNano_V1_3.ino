...
#endif
  NUM_MODES = 12    // Costante che indica il numero totale di modalità di visualizzazione definite nell'enum.
};
...
// Dichiarazioni forward per funzioni definite in altri file .ino
#ifdef EFFECT_ANALOG_CLOCK
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("[WEBSERVER] Accedi a http://" + WiFi.localIP().toString() + ":8080/clock");
    #endif
    ...
    if (alexaOff == 0) {  // Se il display non è stato spento tramite Alexa.
      switch (currentMode) {
        ...
        case MODE_ANALOG_CLOCK:
          if (currentMillis - lastEffectUpdate > 100) {  // Aggiorna ogni 100ms
