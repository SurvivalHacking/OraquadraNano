...
void setup_wifi() {
  ...
  displayWordWifi(32, "WIFI:");         // Mostra "WIFI:".
  gfx->setTextColor(GREEN);
  displayWordWifi(64, "CONNETTITI A:"); // Mostra "CONNETTITI A:".
  displayWordWifi(80, apName);         // Mostra il nome dell'AP.
  gfx->setTextColor(BLUE);
  displayWordWifi(128, "IP: 192.168.4.1"); // Mostra l'IP dell'AP.
...
void setup_alexa() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    espalexa.addDevice("ORAQUADRA_NANO", colorChanged); // Aggiunge un dispositivo Alexa con il nome specificato e la callback per il cambio colore.
    espalexa.begin(); // Inizializza il servizio ESPalexa.
  }
}
...
void colorChanged(uint8_t brightness, uint32_t rgb) {
  // Callback per Amazon Alexa - viene chiamata quando il colore viene cambiato tramite Alexa.
  if (brightness == 0) {
    alexaOff = 1; // Imposta un flag per indicare che Alexa ha spento la luce.
    clearDisplay(); // Pulisce il display.
    return;
  } else {
    alexaOff = 0; // Resetta il flag se la luce è accesa.
  }

  // Converte il valore RGB ricevuto da Alexa nei componenti R, G, B.
  userColor.r = (rgb >> 16) & 0xFF;
  userColor.g = (rgb >> 8) & 0xFF;
  userColor.b = rgb & 0xFF;
